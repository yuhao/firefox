...
#include <time.h>
#include <sys/time.h>
extern int native_depth;
extern JSContext *global_cx;
extern double js_native_time;
...
CallJSNative(JSContext *cx, Native native, const CallArgs &args)
{
    ...
    //YZ --- start ---
	struct timeval native_start, native_end;
	double temp_time;

	if(global_cx && (cx == global_cx))
	{
		native_depth++;
		gettimeofday(&native_start, NULL);
		fprintf(stdout, "Native %d starts @ %f\n", native_depth, ((double)native_start.tv_sec + (double)native_start.tv_usec / 1000000));

		//if(native_depth == 1)
		//{
		//	//fprintf(stdout, "JS Native starts\n");
		//	gettimeofday(&native_start, NULL);
		//}
	}
	...
	if(global_cx && (cx == global_cx))
	{
		gettimeofday(&native_end, NULL);
		fprintf(stdout, "Native %d ends @ %f\n", native_depth, ((double)native_end.tv_sec + (double)native_end.tv_usec / 1000000));
		native_depth--;

		//if(native_depth == 0)
		//{
		//	gettimeofday(&native_end, NULL);
		//	temp_time = (((double)native_end.tv_sec + (double)native_end.tv_usec / 1000000) - ((double)native_start.tv_sec + (double)native_start.tv_usec / 1000000));
		//	js_native_time += temp_time;
		//	//fprintf(stdout, "JS Native ends.  Native time = %f\n", temp_time);
...
CallJSPropertyOp(JSContext *cx, PropertyOp op, HandleObject receiver, HandleId id, MutableHandleValue vp)
{
	...
	if(global_cx && (cx == global_cx))
	{
		...
		if(native_depth == 0)
		{
			...
			temp_time = (((double)native_end.tv_sec + (double)native_end.tv_usec / 1000000) - ((double)native_start.tv_sec + (double)native_start.tv_usec / 1000000));
			js_native_time += temp_time;
			//fprintf(stdout, "JS Prop ends.  Prop time = %f\n", temp_time);
			//fprintf(stdout, "cx %p Native time = %f\n", cx, js_native_time);
		}
	}
    //YZ --- end ---
                       ...
                       JSBool strict, MutableHandleValue vp)
{
	...
	if(global_cx && (cx == global_cx))
	{
		...
		if(native_depth == 1)
		{
			//fprintf(stdout, "JS PropSetter starts\n");
			gettimeofday(&native_start, NULL);
		}
	}

	temp = op(cx, obj, id, strict, vp);
	...
	if(global_cx && (cx == global_cx))
	{
		...
		if(native_depth == 0)
		{
			...
			temp_time = (((double)native_end.tv_sec + (double)native_end.tv_usec / 1000000) - ((double)native_start.tv_sec + (double)native_start.tv_usec / 1000000));
			js_native_time += temp_time;
			//fprintf(stdout, "JS PropSetter ends.  PropSetter time = %f\n", temp_time);
			//fprintf(stdout, "cx %p Native time = %f\n", cx, js_native_time);
		}
	}
    //YZ --- end ---
