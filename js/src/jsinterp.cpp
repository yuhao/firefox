...
int depth = 0;
int native_depth = 0;
JSContext *global_cx = NULL;
double js_native_time = 0.0;
...
js::InvokeKernel(JSContext *cx, CallArgs args, MaybeConstruct construct)
{
	...
	//if(fun->isInterpreted()) script = fun->script();
    ...
    if(strstr(script->filename, "http") == script->filename)
	{
		depth++;
		gettimeofday(&start, NULL);
		fprintf(stdout, "Invoke %d starts @ %f\n", depth, ((double)start.tv_sec + (double)start.tv_usec / 1000000));

		if(depth == 1)
		{
			global_cx = cx;
		//	//fprintf(stdout, "Invoke start %s\n", script->filename);
		//	fprintf(stdout, "cx %p Invoke start %ld\n", cx, ++js_cnt);
		//	gettimeofday(&start, NULL);
		}
	}
    //YZ --- end ---
    ...
    if(strstr(script->filename, "http") == script->filename)
	{
		...
		fprintf(stdout, "Invoke %d ends @ %f\n", depth, ((double)end.tv_sec + (double)end.tv_usec / 1000000));
		depth--;
		...
		//	temp_time = (((double)end.tv_sec + (double)end.tv_usec / 1000000) - ((double)start.tv_sec + (double)start.tv_usec / 1000000));
		//	//js_time += temp_time;
		//	//fprintf(stdout, "cx %p Invoke end %s time = %f\n", cx, script->filename, temp_time);
		//	fprintf(stdout, "cx %p native time = %f\n", cx, js_native_time);
		//	fprintf(stdout, "cx %p Invoke end %ld time = %f\n", cx, js_cnt, temp_time);
                  ...
                  ExecuteType type, StackFrame *evalInFrame, Value *result)
{
    ...
    if(strstr(script->filename, "http") == script->filename)
	{
		depth++;
		gettimeofday(&start, NULL);
		fprintf(stdout, "Execute %d starts @ %f\n", depth, ((double)start.tv_sec + (double)start.tv_usec / 1000000));

		if(depth == 1)
		{
			global_cx = cx;
		//	//fprintf(stdout, "cx %p Execute start script %s\n", cx, script->filename);
		//	fprintf(stdout, "cx %p Execute start %ld\n", cx, ++js_cnt);
		//	gettimeofday(&start, NULL);
		}
	}
    //YZ --- end ---
	...
	{
		gettimeofday(&end, NULL);
		fprintf(stdout, "Execute %d ends @ %f\n", depth, ((double)end.tv_sec + (double)end.tv_usec / 1000000));
		depth--;
		...
		if(depth == 0)
		{
			...
			//temp_time = (((double)end.tv_sec + (double)end.tv_usec / 1000000) - ((double)start.tv_sec + (double)start.tv_usec / 1000000));
			//js_time += temp_time;
			//fprintf(stdout, "cx %p Execute end script %s time = %f\n", cx, script->filename, temp_time);
			//fprintf(stdout, "cx %p native time = %f\n", cx, js_native_time);
			//fprintf(stdout, "cx %p Execute end %ld time = %f\n", cx, js_cnt, temp_time);
